...
- **Pendant un point MJ** : chaque changement de fichier est **commité sur `main` + poussé immédiatement**, dans le même tour, sans branche ni PR (errata §22). Vérifier `git status -sb` après chaque push. Hors point MJ : les corrections s'inscrivent au prochain `codex`.
