...
- **Hook `Stop`** (`hooks/mj-stop.py`) — automatique, aucun modèle : nom scellé dans la sortie joueur (**code 2**), lexique non courant (**code 2**), **file du joueur non vide + bloc d'options (code 2)**, **la bête absente de la prose (code 2)**, `derniere-scene.md`, compteur d'axes, cadence du psy. **C'est un filet, pas un bouclier.**
...
- **LA BÊTE EN SCÈNE — porté par le hook** *(2026-08-22, ordre du joueur)*. `RULE-MJ` §E.1 est une règle dure : **au moins une phrase sur elle dans la PROSE de chaque scène**. Le compteur d'axes ne surveillait que la *déclaration* du MJ ; désormais le hook Stop **lit la prose** — fiche et bloc découpés d'abord, sinon la ligne « ELLE — MAILLE » de la fiche ferait passer n'importe quoi — et **refuse la sortie (code 2)** si elle n'y est pas nommée. *Limite assumée : le hook vérifie qu'elle est **nommée**, pas qu'elle est vivante ; la qualité reste due par le MJ, le nom est le plancher vérifiable.*
- **`file.py` — LA FILE DU JOUEUR** *(2026-08-22, validé par le joueur)*. Quand il enchaîne des options (« 1 puis 4 »), ce n'est pas une préférence, **c'est un engagement** : `file.py --pose 1 4`, puis `--consomme` quand une option est jouée, `--suite` quand elle se joue sur plusieurs messages (combat en échanges, boutique), `--vide` s'il change de programme. **Le hook Stop refuse la sortie (code 2) si un bloc d'options part alors que la file n'est pas vide** — `OPUS` §1ter. *Né de quatre comblements silencieux dans la même session : le MJ avait la file écrite sous les yeux et l'a écrasée, parce qu'un gabarit bat toujours un état quand rien ne l'en empêche.*
