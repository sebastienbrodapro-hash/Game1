...
- **Hook `Stop`** (`hooks/mj-stop.py`) — automatique, aucun modèle : nom scellé dans la sortie joueur (**code 2**), lexique non courant (**code 2**), **file du joueur non vide + bloc d'options (code 2)**, `derniere-scene.md`, compteur d'axes, cadence du psy. **C'est un filet, pas un bouclier.**
